...
const Products = () => {
  ...
  const brandFacets = useSelector(selectBrandFacets);
  ...
  const fetchWithFilters = useCallback(() => {
    ...
    dispatch(fetchCategoryProductsFiltered({
  ...
  // Initial fetch and when filters/category change
  useEffect(() => {
    dispatch(setCategory(category || 'Smartphones'));
    fetchWithFilters();
    updateURLParams();
  }, [category, filters, currentPage, dispatch, fetchWithFilters, updateURLParams]);
      ...
      <div className="bg-white border-b border-gray-200">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
          <nav className="flex items-center space-x-2 text-sm">
            ...
            <span className="text-gray-900 font-medium">{category || 'All Products'}</span>
          </nav>
        </div>
      </div>
                  ...
                  <div className="flex items-center gap-2">
                    <div className="relative flex-1">
                      ...
                      <input
                        ...
                        className="w-full pl-7 pr-3 py-2 bg-gray-50 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500"
                      />
                    </div>
                  ...
                  <div className="space-y-2 max-h-40 overflow-y-auto">
                    {brands.map(brand => (
                      <label key={brand} className="flex items-center gap-2 cursor-pointer group">
                        ...
                        <span className="text-sm text-gray-700 group-hover:text-indigo-600 transition-colors">
                          {brand}
                          <span className="text-gray-400 ml-1">({brandFacets[brand]})</span>
                        </span>
                      </label>
                    ))}
                    {filters.brand && (
          ...
          <main className="flex-1">
            ...
            {status === 'succeeded' && products.length === 0 && (
              <div className="text-center py-24 bg-white rounded-xl">
                ...
                <Link
                  to="/"
                  className="inline-block mt-4 px-6 py-2 border border-indigo-600 text-indigo-600 rounded-lg hover:bg-indigo-50 transition-colors"
                >
                  Back to Home
                </Link>
              </div>
